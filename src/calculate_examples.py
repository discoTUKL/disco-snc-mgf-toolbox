...
if __name__ == '__main__':
    ...
    print(
        OptimizeMitigator(SINGLE_SERVER, print_x=True).grid_search(
            bound_list=[(0.1, 5.0), (0.9, 8.0)], delta=0.1))
    print(
        OptimizeMitigator(SINGLE_SERVER, print_x=True).pattern_search(
    ...
    print(
        OptimizeMitigator(SINGLE_SERVER2, print_x=True).grid_search(
    ...
    print(
        Optimize(SINGLE_SERVER2).grid_search(
            bound_list=[(0.1, 5.0)], delta=0.1))
    ...
    SER_LIST1: List[ConstantRate] = [
        ConstantRate(rate=4.0), ConstantRate(rate=0.5)
    ]

    print(
        FatCrossPerform(
            arr_list=ARR_LIST1, ser_list=SER_LIST1,
    ...
    EXAMPLE = FatCrossPerform(
        arr_list=ARR_LIST2, ser_list=SER_LIST2, perform_param=DELAY_PROB6)
    ...
    DELAY_TIME = PerformParameter(
        perform_metric=PerformEnum.DELAY, value=0.010)

    EXAMPLE_REVERSE = FatCrossPerform(
        arr_list=ARR_LIST2, ser_list=SER_LIST2, perform_param=DELAY_TIME)

    print(
        Optimize(EXAMPLE_REVERSE, print_x=True).grid_search(
            bound_list=[(0.1, 5.0)], delta=0.1))
